(module reward-claim GOV
  ...
  (defschema state
    @doc "Tracks the operational state of the contract"
    paused:bool
  )
  ...
  ; contract management
  (defun set-initial-state:string ()
    @doc "Sets the initial state of the contract"
    (with-capability (OPS)
      (insert contract-state "" { "paused": false})
      (emit-event (UNPAUSED))
      "Unpaused"
    )
  )
  ...
  (defun pause:string ()
    ...
    (with-capability (OPS)
      (enforce-contract-state-unpaused)
      (update contract-state "" {"paused": true})
    (emit-event (PAUSED))
    "Paused"
    )
  )
  ...
  (defun unpause:string ()
    ...
    (with-capability (OPS)
      (enforce-contract-state-paused)
      (update contract-state "" {"paused": false})
      (emit-event (UNPAUSED))
      "Unpaused"
    )
  )
  ...
  (defun current-contract-state:bool ()
    @doc "Returns the current state of the contract"
    (with-default-read contract-state ""
      { 'paused: false }
      { 'paused := paused }
    paused
    )
  ...
  (defun enforce-contract-state-unpaused:bool ()
    (let (
        (cont-st:bool (current-contract-state))
      )
      (enforce (= cont-st false) "Contract is currently paused. Cannot perform operation.")
      )
  )

  (defun enforce-contract-state-paused:bool ()
    (let (
        (cont-st:bool (current-contract-state))
      )
      (enforce (= cont-st true) "Contract is currently unpaused. Cannot perform operation.")
    )
  )
)
